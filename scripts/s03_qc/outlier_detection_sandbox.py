...
#                               parameters
# ----------------------------------------------------------------------
# For now, we'll grab sandbox data that lives in this repository
scl_dir = '/dartfs-hpc/rc/lab/C/CANlab/labdata/projects/spacetop_projects_cue/analysis/physio/physio01_SCL' 
...
pwd = Path.cwd()
main_dir = pwd.parent.parent
scl_dir = join(main_dir, 'data')
qc_fname = join(main_dir, 'data/QC_EDA_new.csv')
...
scl_flist = sorted(glob.glob(join(scl_dir, 'sub-0017_ses-03_run-02_runtype-pain_epochstart--3_epochend-20_baselinecorrect-True_samplingrate-25_physio-eda.txt')))
...
plt.title('outlier validation: 1st iteration with participant "sub-0017')
plt.legend()
plt.show()
...
outlier_data = [column_values[i] if outlier else None for i, outlier in enumerate(outlier_bool)]
plt.scatter(x, outlier_data, color='red', label='Outliers')
plt.title('outlier validation: 2nd iteration with participant "sub-0034"')
plt.legend()
plt.show()
...
outlier_data = [column_values[i] if outlier else None for i, outlier in enumerate(outlier_bool)]
plt.scatter(x, outlier_data, color='red', label='Outliers')
plt.title('outlier validation: 3rd iteration with participant "sub-0051')
plt.legend()
plt.show()

# %%----------------------------------------------------------------------
#  outlier removal and interpolation - As of 02/07/2024, we will not interpolate
# ----------------------------------------------------------------------
