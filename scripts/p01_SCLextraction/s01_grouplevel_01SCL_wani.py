...
# NOTE: if you're using a virtual env, install spacetop_prep first
...
def main():
    ...
    beh_fname = '/Users/h/Documents/projects_local/spacetop-prep/spacetop_prep/physio/demo/data/physio03_bids/sub-0051/ses-01/sub-0051_ses-01_task-social_run-02-pain_beh.csv'
    physio_fpath = '/Users/h/Documents/projects_local/spacetop-prep/spacetop_prep/physio/demo/data/physio03_bids/sub-0051/ses-01/sub-0051_ses-01_task-cue_run-02-pain_recording-ppg-eda-trigger_physio.tsv'
    meta_fname = '/Users/h/Documents/projects_local/spacetop-prep/spacetop_prep/physio/demo/spacetop_task-social_run-metadata.csv'
    ...
    metadata = meta_fname
    dictchannel_json = dictchannel_json
    slurm_id = 1  # e.g. 1, 2
    stride = 1 # e.g. 5, 10, 20, 1000
    zeropad = 4  # sub-0016 -> 4
    task = 'task-cue'  # e.g. 'task-social' 'task-fractional' 'task-alignvideos'
    samplingrate = 2000  # e.g. 2000
    SCL_epoch_start = -3
    SCL_epoch_end = 15
    ttl_index = 1
    remove_subject_int = [99]
    ...
    # NOTE:  TONIC ________________________________________________________________________________
        ...
        tonic_length, scl_raw, scl_epoch = utils.preprocess.extract_SCL_custom(df=physio_df,
                                eda_col='physio_eda_blcorrect', event_dict=event_stimuli, samplingrate=2000,
                                SCL_start=SCL_epoch_start, SCL_end=SCL_epoch_end, baseline_truefalse=False, highpass_filter = False)
    ...
    #  NOTE: concatenate dataframes __________________________________________________________________________
        ...
        eda_level_timecourse = utils.preprocess.resample_scl2pandas_ver2(scl_output = scl_raw, 
                                                                         ...
                                                                         desired_sampling_rate = resample_rate)
