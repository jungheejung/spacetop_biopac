...
% normalize or demean ratings? 

% THOUGHTS: --> TODO: organize with heejung
% pick up on 
% event time window for 23 seconds: DONE
...
for i = 1:numel(sub_list_clean)
    ...
    y_exp{i} = data.event02_expect_angle(data.src_subject_id == sub_list_clean(i)); % unpleasantless (bipolar) -> expectation rating
...
% 3 seconds are used for baseline (= calculate the mean within subject per 
...
dat.exp = dat.int;
dat.exp.Y = cat(1,dat_exp{:,1});%cat(1,dat_unp{:,2});  % separate code & predict rating from medium intensity , second column of temp
...
[~, pcr_stats.unp, ~] = predict(dat.unp, 'algorithm_name', 'cv_pcr', 'nfolds', whfolds, 'numcomponents', 6, 'bootweights', 'bootsamples', 10000); % run it again

save(fullfile(datdir, 'SCR_prediction_dat_112816.mat'), '-append', 'pcr_stats', 'pred_outcome_r', 'rmse');


%%
% ========================= 3.4: plot for intensity =======================
...
y_exp = [cat(2,scr.dat_unp{:,1}) cat(2,scr.dat_unp{:,3})];
