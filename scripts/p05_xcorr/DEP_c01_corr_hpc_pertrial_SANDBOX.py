...
save_top_dir = '/Users/h/Desktop/TMP'
# %% 0. parameters
...
for i, physio_fname in enumerate(physio_flist):
    ...
    roi_data = []
    for roi_ind in np.arange(3):#range(time_series.shape[1]):
        # remove outlier
...
# extract physio and fmri data based on onset time
        data1 = physio_standardized#physio_shave.squeeze()
        data2 = fmri_standardized#fmri_shave.squeeze()
        physio_stim = []
        fmri_stim = []

        built_in_delay = 5
        delay_seconds = built_in_delay * Fs
        ...
        for i, (start, stop ) in enumerate(zip(event_stimuli_start, event_stimuli_stop)):
            ...
            physio_segment = data1.loc[int(start)-padding_samples + int(delay_seconds):int(stop)+padding_samples + int(delay_seconds)].to_numpy()
            fmri_segment = data2.loc[int(start)-padding_samples+ int(delay_seconds):int(stop)+padding_samples+ int(delay_seconds)].to_numpy()
            physio_stim.append(physio_segment)
            fmri_stim.append(fmri_segment)
        ...
        for i in np.arange(len(physio_stim)):
            fmri_data = np.array(fmri_stim[i].flatten())
            physio_data = np.array(physio_stim[i].flatten())
            # 4-2. plot parameters
            ...
            # data1 = physio_iti[i]
            # data2 = fmri_iti[i]
            tvec = np.arange(0, len(physio_data) / Fs, 1/Fs)
            tvec = tvec[:len(physio_data)]
            # 4-2.A: Plot raw signals ______________________________
            ax1.plot(tvec, physio_data, 'r', linewidth=1, alpha=0.7)
            ax1.plot(tvec, fmri_data, 'b', linewidth=1, alpha=0.7)
            ...
            min_signal_length = min(len(physio_data), len(fmri_data))
            ws = int(Fs * 10)#max(min_signal_length , 1)  # Ensure ws is at least 1
            window = hann(ws)
            noverlap = ws // 2
            # ws = len(physio_data) // 2
            # window = hann(ws)
            # noverlap = ws // 2
            nfft = len(tvec)
            print(f"length ws,window,nfft  {ws} {len(window)} {nfft} {len(physio_data)}")
            f, Pxx = welch(physio_data, Fs, window=window, noverlap=noverlap, nfft=nfft)
            _, Pyy = welch(fmri_data, Fs, window=window, noverlap=noverlap, nfft=nfft)
            ...
            f, Pxy = csd(physio_data, fmri_data, Fs, window=window, noverlap=noverlap, nfft=nfft)
            ...
            maxlags = int(Fs * 10)# len(physio_data) // 2#int(Fs * 30)
            acf = correlate(physio_data, fmri_data, mode='full', method='auto') # matlab: xcorr
            # acf /= len(data1)  # Normalizing
            norm_factor = np.sqrt(np.sum(physio_data**2) * np.sum(fmri_data**2))
            ccf = acf / norm_factor
            ...
            # 4-2.E: Coherence
            f_coh, Cxy = coherence(physio_data, fmri_data, Fs, window=hann(ws), noverlap=noverlap, nfft=nfft)
            ax5.plot(f_coh, Cxy)
